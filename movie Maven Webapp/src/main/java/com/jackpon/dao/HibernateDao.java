...
public class HibernateDao extends SuperDAO{
	...
	public List<Movies> getAllMovies(int page) {
		int firstResult=(page-1)*Movies.PAGE_SIZE;
		int maxResults=Movies.PAGE_SIZE;
		DetachedCriteria criteria = DetachedCriteria.forClass(Movies.class);
		criteria.add(Restrictions.eq("type", "movies"));
		List<Movies> list=(List<Movies>) this.getHibernateTemplate().findByCriteria(criteria, firstResult, maxResults);
	...
	public List<Movies> getListForPage(final String hql, final int offset,final int length){
		List<Movies> list = (List<Movies>) getHibernateTemplate().execute(
